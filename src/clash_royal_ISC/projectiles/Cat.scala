...
class Cat(attackDamage: Int ,position: Vector2, target: Entity) extends Projectile(attackDamage, position, target) {
  ...
  override val SPAWN_SOUND: SoundSample = new SoundSample("res/sounds/projectiles/cat/spawn.mp3")
  override val HIT_SOUND: SoundSample = new SoundSample("res/sounds/projectiles/cat/hit.ogg")
}